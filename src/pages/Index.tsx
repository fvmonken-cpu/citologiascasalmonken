...
const Index = ()=>{
    ...
    // Rastreia o ID do último usuário que gerou redirecionamento para evitar reset
    // de página quando o Supabase re-emite SIGNED_IN ao voltar para a aba
    const redirectedForUserRef = useRef<string | null>(null);
    useEffect(()=>{
        if (user && !loading && redirectedForUserRef.current !== user.id) {
